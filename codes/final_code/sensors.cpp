...
int main(void) {
	...
	//delay(15);
	pthread_create(&t3, NULL, shape, (void *)&args_Lane);
	delay(15);
	...
	while(1) {
		...
		if(nValue == 0) {
			...
			if(count == 1) {
				...
				cout << "obstacle avoidanca " << endl;
				setCommand(TOP_IR_PRIVILEGE_LEVEL, SPEED_UP);
				...
				delay(500);
				setCommand(TOP_IR_PRIVILEGE_LEVEL, LEFT_CURVE_TURN_20_60);
				delay(2000);
				setCommand(TOP_IR_PRIVILEGE_LEVEL, RIGHT);
				delay(250);
				/*
	...
	*/
	...
	while (iter != result.end()) {
		...
		if (temp.getName() == "right") { // down speed 
			...
			setCommand(SHAPE_DETECTOR_PRIVILEGE_LEVEL, NOTHING);
		}
...
// g++ sensors.cpp MotorControl.cpp -o sen -lwiringPi -pthread
