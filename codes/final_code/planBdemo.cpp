...
void detect(Mat src) {
	...
	while (iter != result.end()) {
		Object& temp = *iter;
		if(temp.getName() == "left") {
		    setCommand(TOP_IR_PRIVILEGE_LEVEL, LEFT_CURVE_TURN_20_75);
		    delay(200);
		    setCommand(TOP_IR_PRIVILEGE_LEVEL, STOP);
		    delay(5000);
		} /*
...
// g++ planBdemo.cpp MotorControl.cpp ShapeDetector.cpp ColorSplitter.cpp CollectionAggregator.cpp Object.cpp -o planBdemo -I/usr/local/include -L/usr/local/lib -L/opt/vc/lib -lraspicam -lraspicam_cv -lmmal -lmmal_core -lmmal_util `pkg-config --cflags --libs opencv` -lpthread -lwiringPi
