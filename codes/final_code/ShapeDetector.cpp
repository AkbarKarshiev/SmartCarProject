...
void getBlue(Mat src, int thresh);
...
std::list<Object> ShapeDetector::getShapes(String color, Mat src, int thresh) {
	...
	for (int i = 0; i < contours.size(); i++) {
		...
		if (color == "red" && approx.size() != 4 && std::fabs(cv::contourArea(contours[i])) > 1000 ) {
			// do smth
			cout <<"Big red" <<std::fabs(cv::contourArea(contours[i]))<<endl;
			setLabel(result, "bigred", contours[i]);
			cv::Point position = getPosition(contours[i]);
			Object o("bigred", color, position);
			objects.push_back(o);
		}
		
		if (std::fabs(cv::contourArea(contours[i])) > 1000 && std::fabs(cv::contourArea(contours[i])) < 700 )
			continue;
		...
		//rectangle and hexagon
			...
			if (vertices == 4 && minCos >= -0.1 && maxCos <= 0.3) {
				...
				if (hierarchy[i][2] != -1 && color == "blue") {
					cv::approxPolyDP(cv::Mat(contours[i + 1]), approx, cv::arcLength(cv::Mat(contours[i + 1]), true) * 0.02, true);

					//skip small ..
					if (std::fabs(cv::contourArea(contours[i])) < 600)
						continue;
...
std::string ShapeDetector::getBlue(String color, Mat src, int thresh) {
	Mat gray;
	cvtColor(src, gray, CV_BGR2GRAY);
	//convert to gray, blur and apply a threshold
